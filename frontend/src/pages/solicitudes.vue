<template>
  <v-container v-if="accesoPermitido">
    ...
    <ItemsTable :headers="headers" :items="loans" :search="search" title="Solicitudes">
      <!-- Fecha del pedido -->
      ...
      <template #item.actions="{ item }">
        <v-btn color="green" @click="acceptLoan(item.id)">
          Aceptar
        </v-btn>
        <v-divider class="mx-2" vertical />
        <v-btn color="red" @click="rejectLoan(item.id)">
          Rechazar
        </v-btn>
      </template>
    </ItemsTable>
...
<script setup lang="ts">
  ...
  function getUserEmail (id: number) {
    return users.value.find(u => u.id === id)?.email || 'Desconocido'
  }

  async function acceptLoan (id: number) {
    const token = localStorage.getItem('auth_token')
  ...
  async function rejectLoan (id: number) {
    const token = localStorage.getItem('auth_token')
