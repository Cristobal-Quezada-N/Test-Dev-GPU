<template>
  <v-app>
    <!-- Navigation Drawer -->
    <v-navigation-drawer
v-model="appStore.drawer" class="elevation-2" permanent :rail="rail" @mouseenter="rail = false"
      ...
      <template v-if="appStore.isAdmin">
        ...
        <v-list density="compact" nav>
          <v-list-item
v-for="item in userNavigationItems" :key="item.title"
            :class="{ 'v-list-item--active': $route.path === item.to }" :prepend-icon="item.icon" rounded="lg"
...
v-for="item in userNavigationItems" :key="item.title"
      ...
      <v-list class="mt-auto" density="compact" nav>
        <v-list-item
class="mb-2" prepend-icon="mdi-logout" rounded="lg" subtitle="Salir del sistema"
          title="Cerrar Sesión" @click="handleLogout" />
      </v-list>
    </v-navigation-drawer>

    <!-- Top App Bar -->
    <v-app-bar class="border-b" color="surface" elevation="1">
    ...
    <!-- Loading Overlay -->
    <v-overlay v-model="appStore.isLoading" class="align-center justify-center">
